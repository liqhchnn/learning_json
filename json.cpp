...
Json::operator bool() const {
    if(m_type != json_bool) {
        throw new std::logic_error("type error, not bool value"); //这里new，之后会回收吗？
    }
...
Json::operator int() const {
    if(m_type != json_int) {
        throw new std::logic_error("type error, not int value"); //这里new，之后会回收吗？
    }
...
Json::operator double() const {
    if(m_type != json_double) {
        throw new std::logic_error("type error, not double value"); //这里new，之后会回收吗？
    }
...
Json::operator string() const {
    if(m_type != json_string) {
        throw new std::logic_error("type error, not string value"); //这里new，之后会回收吗？
    }
    return *m_value.m_string;
}

Json & Json::operator[] (int index) {
    if (m_type != json_array) {  //为什么会不是数组类型
        m_type = json_array;
    ...
    if (index >= size) {  //扩容   ?为什么会大于size，为什么不报错 Q:这里初始化的vecor，size为0，通过[i]访问是会报错的
...
Json & Json::operator[] (const char *key) {  //转成字符串，再去调用string的重载[]
    std::string name(key);
    return (*(this))[name];
}
...
bool Json::operator== (const Json &other) {
    ...
    switch (m_type) {
        ...
        case json_array:  //为了简单，直接判断，有问题
            return m_value.m_array == other.m_value.m_array;
        case json_object://为了简单，直接判断，有问题
...
bool Json::asBool () const {
    if(m_type != json_bool) throw new std::logic_error("type error, not bool value");
    return m_value.m_bool;
}

int Json::asInt () const {
    if(m_type != json_int) throw new std::logic_error("type error, not int value");
    return m_value.m_int;
}

double Json::asDouble () const {
    if(m_type != json_double) throw new std::logic_error("type error, not double value");
    return m_value.m_double;
}

string Json::asString () const {
    if(m_type != json_string) throw new std::logic_error("type error, not string value");
    return *(m_value.m_string);
}
